...
namespace tasarım1.Attribute
{
    public class FilterLog : ActionFilterAttribute
    {
        ...
        private void Log(ActionExecutingContext filterContext)
        {
            ...
            var areasName = filterContext.RouteData.Values["area"];
            ...
            {
                actionController = areasName + "/" + controllerName + "/" + actionName + "/id?=" + idName;
            }

            var date = DateTime.Now;
            var user = filterContext.HttpContext.Session["Login"] == null 
                ? filterContext.HttpContext.Session["AdminLogin"] 
                : filterContext.HttpContext.Session["Login"];
            ...
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("spUserLogger", conn);
